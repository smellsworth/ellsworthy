declare module 'polka' {
  ...
  export interface Polka<T = Request> extends Trouter<RequestHandler<T>> {
    ...
    get(pattern: string, ...handlers: Middleware<T>[]): this
    post(pattern: string, ...handlers: Middleware<T>[]): this
    put(pattern: string, ...handlers: Middleware<T>[]): this
    patch(pattern: string, ...handlers: Middleware<T>[]): this
    delete(pattern: string, ...handlers: Middleware<T>[]): this

    readonly handler: RequestHandler<T>
    readonly routes: { [key: string]: { old: string; type: number; val: string; end: string }[][] }
